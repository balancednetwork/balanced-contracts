...
filename = '/home/sudeep/contracts-private/test/scenarios/loan-addCollateral.json'
...
class TestLoan(IconIntegrateTestBase):
    ...
    def setUp(self):
        ...
        # deploy SCORE
        governance = self._deploy_score(self.contracts['governance'], {}, self.wallet, 0)
        for address in DEPLOY:
            self.SCORE_PROJECT = self.SCORES + "/" + address
            print('Deploying ' + address + ' Contract in Testnet')
            self.contracts[address] = self._deploy_score(self.contracts[address], {'_governance': governance}, self.wallet, 0)['scoreAddress']
    ...
    def _deploy_score(self,contract, params, _test1, update) -> dict:
        if update:
            dest = contract['SCORE']
        else:
