...
class Rewards(IconScoreBase):
    ...
    @external
    def set_token_address(self, _address: Address) -> None:
    ...
    @external
    def set_start_timestamp(self, _timestamp: int) -> None:
        if self.msg.sender != self.owner:
            revert(f"Only owner can set the token address")
    ...
    @external(readonly=True)
    def get_data_source_names(self) -> list:
        data_source_names = []
    ...
    @external
    def add_new_data_source(self, _data_source_name: str, _contract_address: Address):
    ...
    @external(readonly = True)
    def get_data_sources(self, _data_source_name: str) -> dict:
        response = {}
        if _data_source_name in self._data_source_names:
            response = get_data_from_data_source(_data_source_name, self._data_source_db)
        return response

    def _reward_distribution(self, _data_source_name: str) -> None:
        self._data_source_db[_data_source_name]._distribute()
    ...
    def distribute(self) -> bool:
        ...
        for data_source_name in self._data_source_names:
            ...
            if data_source[day] < self._get_day():
                self._reward_distribution(data_source_name)
