...
class TestBWT(ScoreTestCase):
    def setUp(self):
        ...
        self.owner_address = Address.from_string(f"hx{'1563' * 10}")
        self.baln_address = Address.from_string(f"hx{'1578' * 10}")

        account_info = {
            self.test_account3: 10 * 10 ** 18,
            self.test_account4: 10 * 10 ** 18,
            self.baln_address: 10 * 10 ** 18,
            self.owner_address: 10 * 10 ** 18}
        ...
        self.score = self.get_score_instance(WorkerToken, self.owner_address,
    ...
    def test_set_governance_not_contract(self):
        try:
            ...
            self.score.setGovernance(self.test_account4)
    ...
    def _set_baln_address(self):
        self.set_msg(self.admin_address)
        self.score.setBaln(self.baln_address)

    def test_set_baln(self):
        self._set_admin()
        self._set_baln_address()

        self.assertEqual(self.baln_address, self.score._baln_token.get())
    ...
    def test_admin_transfer_non_owner(self):
        ...
        try:
            self.score.adminTransfer(_from=self.owner_address, _to=self.test_account4, _value=10)
        except SenderNotAuthorized:
            self.assertTrue(True)
        else:
            self.assertTrue(False)
    ...
    def test_admin_transfer(self):
        ...
        pre_balance_from = self.score.balanceOf(self.owner_address)
        pre_balance_to = self.score.balanceOf(self.test_account4)

        self.set_msg(self.admin_address)
        self.score.adminTransfer(_from=self.owner_address, _to=self.test_account4, _value=10)

        post_balance_from = self.score.balanceOf(self.owner_address)
    ...
    def test_distribute(self):
        ...
        transfer_amount = int(self.score.balanceOf(self.owner_address) * 0.25)

        self.set_msg(self.admin_address)
        self.score.adminTransfer(_from=self.owner_address, _to=self.test_account4,
        ...
        self.assertEqual(750000000000000000000000, mock_object.logs[self.owner_address])
    ...
    def test_token_fallback(self):
        ...
        self.set_msg(self.baln_address)
