...
from token_contracts.bnDOGE.bnDOGE import BalancedDoge, EXA
from token_contracts.bnDOGE.utils.checks import SenderNotScoreOwnerError, SenderNotAuthorized, SenderNotGovernance
from token_contracts.bnDOGE.tokens.IRC2 import ZeroValueError
...
class TestBNXLM(ScoreTestCase):
    def setUp(self):
        ...
        self.test_account = Address.from_string(f"hx{'1434' * 10}")
        self.owner_address = Address.from_string(f"hx{'8452' * 10}")
        self.governance_address = Address.from_string(f"hx{'8942' * 10}")

        account_info = {
            self.owner_address: 10 ** 21,
            self.admin_address: 10 ** 21,
            self.oracle_address: 10 ** 21,
            self.test_account: 10 ** 21,
        ...
        self.score = self.get_score_instance(BalancedDoge, self.owner_address,
    ...
    def test_set_admin_not_governance(self):
        try:
            self.set_msg(self.test_account)
    ...
    def test_priceInLoop(self):
        ...
        icx_price = mock_obj.get_reference_data("USD", "ICX")
        price_data = mock_obj.get_reference_data(self.score._peg.get(), "USD")
    ...
    def test_last_price_in_loop(self):
        ...
        self.assertEqual(30366583579709408, return_data)
