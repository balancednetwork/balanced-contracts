...
transaction = TransactionBuilder().from_(wallet.get_address()).to(user1.get_address()).value(1000000 * ICX).step_limit(
...
contracts = {'loans': {'zip': 'core_contracts/loans.zip', 'SCORE': 'cx55f2ab7a026bd070d117233f32fbc614bd904a20'},
             'staking': {'zip': 'core_contracts/staking.zip', 'SCORE': 'cxd0c708c7c7c2ed97b10bc0a1772f7036d351a5c5'},
             'dividends': {'zip': 'core_contracts/dividends.zip',
                           'SCORE': 'cx3d1fa0db471e68dc5df110455660a1c476e46eec'},
             'reserve': {'zip': 'core_contracts/reserve.zip', 'SCORE': 'cx8d447de699e8c40bafe0fd35d498cd3c58c99425'},
             'daofund': {'zip': 'core_contracts/daofund.zip', 'SCORE': 'cxfd51e9df2e7a10df873009a2a6bf80c13a0508af'},
             'rewards': {'zip': 'core_contracts/rewards.zip', 'SCORE': 'cx5c3c476f9a097589d7108fe2e9b71ada7161ad78'},
             'dex': {'zip': 'core_contracts/dex.zip', 'SCORE': 'cxd9f12ee8e9a3d0774e929b3118c568555a419385'},
             'governance': {'zip': 'core_contracts/governance.zip',
                            'SCORE': 'cx5c0170c8a2059a8678e65297fd4188b3de631b74'},
             'oracle': {'zip': 'core_contracts/oracle.zip', 'SCORE': 'cx7171e2f5653c1b9c000e24228276b8d24e84f10d'},
             'sicx': {'zip': 'token_contracts/sicx.zip', 'SCORE': 'cx51b3add2ab11c541b628fd8eca22dd362b11f54c'},
             'bnUSD': {'zip': 'token_contracts/bnUSD.zip', 'SCORE': 'cx228a83c7dabe2a34424de5d963944b1a1e55d09e'},
             'baln': {'zip': 'token_contracts/baln.zip', 'SCORE': 'cxcacc2eef044822a9f000a1566f3585fe26f35af2'},
             'bwt': {'zip': 'token_contracts/bwt.zip', 'SCORE': 'cx2c79be5145135b056d49990ed3259adfb4acef75'}}
...
transaction = CallTransactionBuilder().from_(btest_wallet.get_address()).to(contracts['governance']['SCORE']).value(
    0).step_limit(10000000).nid(NID).nonce(100).method("setMarketName").params(
    {'_pid': 3, '_name': 'BALNbnUSD'}).build()
...
send_tx('bnUSD', 0, 'transfer', {'_to': contracts['loans']['SCORE'], '_value': 50 * ICX,
                                 '_data': json.dumps({"method": "_retire_asset", "params": {}}).encode()}, btest_wallet)
call_tx('loans', 'getAvailableAssets', {})
...
for case in test_cases['stories']:
    ...
    transaction = CallTransactionBuilder().from_(btest_wallet.get_address()).to(_to).value(val).step_limit(
        10000000).nid(NID).nonce(100).method(meth2).params(second_params).build()
    signed_transaction = SignedTransaction(transaction, btest_wallet)
    tx_hash = icon_service.send_transaction(signed_transaction)
    sleep(1)
...
for _ in range(100):
    ...
    transaction = CallTransactionBuilder().from_(user2.get_address()).to(contracts['loans']['SCORE']).value(
...
params = {}
call = CallBuilder().from_(user2.get_address()).to(contracts['loans']['SCORE']).method("borrowerCount").params(
    params).build()
...
for i in range(1, borrowerCount + 1):
    ...
    call = CallBuilder().from_(user2.get_address()).to(contracts['loans']['SCORE']).method("getPositionByIndex").params(
        params).build()
    position = icon_service.call(call)
    addresses.append(position['address'])

params = {'_holders': addresses}
call = CallBuilder().from_(user2.get_address()).to(contracts['rewards']['SCORE']).method("getBalnHoldings").params(
    params).build()
...
for contract in ['rewards', 'reserve', 'bwt']:
    ...
    call = CallBuilder().from_(user2.get_address()).to(contracts['baln']['SCORE']).method("balanceOf").params(
        params).build()
