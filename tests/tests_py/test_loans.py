...
transaction = TransactionBuilder()    .from_(wallet.get_address())    .to(user2.get_address())    .value(10000 * ICX)    .step_limit(1000000)     .nid(NID)     .nonce(2)     .version(3)     .build()
...
transaction = CallTransactionBuilder()    .from_(user1.get_address())    .to(contracts['bnUSD']['SCORE'])    .value(0)    .step_limit(10000000)    .nid(NID)    .nonce(100)    .method("transfer")    .params(params)    .build()
...
transaction = CallTransactionBuilder()    .from_(user2.get_address())    .to(contracts['bnUSD']['SCORE'])    .value(0)    .step_limit(10000000)    .nid(NID)    .nonce(100)    .method("transfer")    .params(params)    .build()
...
params = {}
call1 = CallBuilder().from_(wallet.get_address())                     .to(contracts['loans']['SCORE'])                     .method("borrowerCount")                     .params(params)                     .build()
...
for i in range(total_borrowers):
    ...
    transaction = CallTransactionBuilder()        .from_(wallet.get_address())        .to(contracts['loans']['SCORE'])        .value(0)        .step_limit(10000000)        .nid(NID)        .nonce(100)        .method("updateStanding")        .params(params)        .build()
    signed_transaction = SignedTransaction(transaction, wallet)
    tx_hash = icon_service.send_transaction(signed_transaction)
    sleep(1)
...
for case in test_cases['stories']:
    ...
    transaction = CallTransactionBuilder()        .from_(user1.get_address())        .to(_to)        .value(val)        .step_limit(10000000)        .nid(NID)        .nonce(100)        .method(meth1)        .params(first_params)        .build()
    signed_transaction = SignedTransaction(transaction, user1)
    tx_hash = icon_service.send_transaction(signed_transaction)
    sleep(2)

    transaction = CallTransactionBuilder()        .from_(user2.get_address())        .to(_to)        .value(val)        .step_limit(10000000)        .nid(NID)        .nonce(100)        .method(meth2)        .params(second_params)        .build()
    signed_transaction = SignedTransaction(transaction, user2)
    tx_hash = icon_service.send_transaction(signed_transaction)
    sleep(1)
